...
namespace Chem3D
{
    public static class MeshExtensions
    {
        ...
        // Add a cylindric line defined by two points.
        public static void AddLine(this MeshGeometry3D mesh, Point3D first,
            Point3D second, double thickness = 0.08)
        {
            Point3D center = first;
            Vector3D dir = new Vector3D(second.X - first.X,
                second.Y - first.Y, second.Z - first.Z);
            Vector3D polygonDir1 = new Vector3D(-(dir.Y * 1 + dir.Z * 1), 1, 1);
            Vector3D polygonDir2 = new Vector3D(dir.Y * polygonDir1.Z - dir.Z * polygonDir1.Y,
                -(dir.X * polygonDir1.Z - dir.Z * polygonDir1.X),
                dir.X * polygonDir1.Y - dir.Y * polygonDir1.X);
            Point3D[] polygon = G3.MakePolygonPoints(20, center,
                new Vector3D(polygonDir1.X / polygonDir1.Length * thickness,
                polygonDir1.Y / polygonDir1.Length * thickness,
                polygonDir1.Z / polygonDir1.Length * thickness),
                new Vector3D(polygonDir2.X / polygonDir2.Length * thickness,
                polygonDir2.Y / polygonDir2.Length * thickness,
                polygonDir2.Z / polygonDir2.Length * thickness));
            mesh.AddCylinder(polygon, dir, true);
        }
